...
void shooter(int id, int seed_fd_rd, int score_fd_wr)
{
	...
	while(read(seed_fd_rd, &seed, sizeof(int)) <= 0);
	srand(seed);
	fprintf(stderr, "player %d: My seed is %d\n", id, seed);
	score = (int)(1.0f+((float)(MAX_RANDOM-1)*((float)rand() / (float)RAND_MAX)));
	fprintf(stderr, "player %d: I scored %d (PID = %ld\n", id, score, (long)pid);

	if(!write(score_fd_wr, &score, sizeof(int)))
	{
		perror("Failed to write score!\n");
		exit(EXIT_FAILURE);
	}
